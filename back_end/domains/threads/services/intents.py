...
def build_thread_session_intent(ticket, thread_uid: str) -> str:
    ...
    return (
        ...
        "- NEVER use `opensweep_platform_add_comment` in this thread. "
        "Comments are a separate surface — posting questions or the plan "
        "there splits the conversation and the platform will not see them. "
        "Questions go through `ask_user`; the plan goes through "
        "`submit_thread_plan` (the ONLY place the platform reads it from); "
        "everything else is said right here in the conversation.\n"
        "\n"
