...
def build_thread_session_intent(ticket, thread_uid: str) -> str:
    ...
    return (
        ...
        "plan_markdown, steps). `steps` is the ordered implementation "
        "checklist (5-15 short titles) — it becomes the ticket's tracked "
        "checklist that you will mark off while implementing. The markdown "
        "must cover the concrete approach, the files to touch, subagent "
        "strategy if any, and a 'How to verify' section. Keep iterating on "
        "the plan in this conversation until the user approves or tells you "
        "to stop.\n"
