...
/**
 * Files tab of the run detail page: a two-pane explorer over
 * GET /runs/{uid}/changes. Left: the changed files (or the full workspace
 * tree) as a nested tree; right: a GitHub-style diff for the selection.
 * Refetches whenever `refreshKey` bumps (turn boundaries) — only the very
 * first load shows a loading state, later refreshes swap data in place.
 */
...
async function fetchChanges() {
  ...
  try {
    const res = await runs.getChanges(props.runUid)
    data.value = res
    error.value = null
        ...
        <div class="min-w-0 border-b md:border-b-0 md:border-r">
          ...
          <div class="max-h-[560px] overflow-auto py-1">
            <div v-if="data.source === 'none'" class="px-3 py-6 text-xs text-muted-foreground">
              No workspace and no snapshot for this run yet.
            </div>
            <div v-else-if="!visibleRows.length" class="px-3 py-6 text-xs text-muted-foreground">
              {{ mode === 'changed' ? 'No files changed in this run yet.' : 'No files in the workspace tree.' }}
            </div>
